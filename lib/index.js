...
var loginWithBrowser = function loginWithBrowser() {
    return new Promise(function (resolve, reject) {
        ...
        var loginFinshed = false;

        var loginPopup = window.open(loginWindowURL, '_blank', 'location=no');

        var loginPopupCompleted = function loginPopupCompleted() {
            loginFinshed = true;
            clearInterval(loginPopupInterval);
        };

        var handleOauthResponse = function handleOauthResponse(url) {
        ...
        var loginPopupInterval = setInterval(function (event) {
            try {
